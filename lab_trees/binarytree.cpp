...
void BinaryTree<T>::printLeftToRight(const Node* subRoot) const
{
    if (subRoot != NULL) {
      ...
      string str = to_string(subRoot->elem) + " ";
      cout << str;
...
void BinaryTree<T>::printPaths() const
{
  ...
  printPaths((const Node*) root, storage);
}

template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, vector <string> storage) const 
{
  if (subRoot != NULL) {
    string str = to_string(subRoot->elem) + " ";
    ...
    if (subRoot->left == NULL && subRoot->right == NULL) {
      ...
      printPaths(subRoot->left, storage);
      printPaths(subRoot->right, storage);
    }
  }
}
...
{
  //cout << "This is my sum: " << sumDistances(root) << endl;
  return sumDistances(root, 0);
}

template <typename T>
int BinaryTree<T>::sumDistances(const Node* subRoot, int n) const
{
  if (subRoot == NULL) {
    return 0;
  }
